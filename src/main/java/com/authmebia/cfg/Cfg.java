...
public class Cfg {
    ...
    public static void withPlayerContext(String playerName, Runnable task) {
        ...
        try {
            ...
            if (previous == null) {
                PLAYER_CONTEXT.remove();
            } else {
                PLAYER_CONTEXT.set(previous);
            }
        }
    }
    ...
    public boolean premiumAutologinEnabled() {
        return config.getBoolean("auto.premium_autologin", false);
    }

    public boolean sessionAutologinEnabled() {
        return config.getBoolean("auto.session_autologin", true);
    }

    public boolean bedrockAutologinEnabled() {
    ...
    public boolean bedrockDialogOverrideEnabled() {
        return config.getBoolean("dialog.bedrock.enabled", false);
    }

    public int bedrockButtonWidth() {
        return clampWidth(config.getInt("dialog.bedrock.button_width", 300));
    }

    public int bedrockInputWidth() {
        return clampWidth(config.getInt("dialog.bedrock.input_width", 300));
    }

    public Mode bedrockAuthMode() {
        if (!bedrockDialogOverrideEnabled()) return javaAuthMode();
        String raw = config.getString("dialog.bedrock.auth_mode_override", "");
        if (raw == null || raw.isBlank()) return javaAuthMode();
    ...
    public boolean loginAttemptsEnabled() {
        return config.getBoolean("login_attempts.enabled", true);
    }
    ...
    public Component premiumResetTitle() {
        return parse(config.getString("premium.title", "<gold>Set Your Password</gold>"));
    }

    public Component premiumResetContent() {
        return parse(config.getString("premium.content",
                "<gray>You were signed in automatically. Please set a password for your account.</gray>"));
    }

    public String premiumResetNewPasswordLabel() {
        return config.getString("premium.new_password_label", "New password");
    }

    public String premiumResetConfirmPasswordLabel() {
        return config.getString("premium.confirm_password_label", "Confirm password");
    }

    public Component premiumResetSubmitButton() {
        return parse(config.getString("premium.submit_button", "<green>Set Password</green>"));
    }

    public String premiumResetSubmitSound() {
        return config.getString("premium.submit_sound", "");
    }

    public Component premiumResetSuccessMessage(String playerName) {
        return parse(replacePlayer(config.getString("premium.success_message",
                "<green>Password set successfully.</green>"), playerName));
    }

    public Component premiumResetMismatchMessage() {
        return parse(config.getString("premium.mismatch_message",
                "<red>Passwords do not match. Try again.</red>"));
    }
    ...
    private CustomScreen buildCustomScreen(Map<?, ?> map) {
        ...
        boolean enabled = readBoolean(map, "enabled", true);
        String contextPlayerName = PLAYER_CONTEXT.get();
        Component title = parse(replacePlayer(readString(map, "title", "Notice"), contextPlayerName));
        if (title == null) title = net.kyori.adventure.text.Component.text("Notice");
        Component content = parse(replacePlayer(readString(map, "content", ""), contextPlayerName));
        ...
        if (rawBtns instanceof List<?> btnList) {
            for (Object o : btnList) {
                ...
                Component label = parse(replacePlayer(readString(bm, "label", "OK"), contextPlayerName));
    ...
    public List<com.authmebia.notifications.Toast> toasts() {
        ...
        for (Map<?, ?> map : config.getMapList("notifications.toasts")) {
            com.authmebia.notifications.Toast t = buildToast(map);
            if (t != null) result.add(t);
        }
        return result;
    }

    private com.authmebia.notifications.Toast buildToast(Map<?, ?> map) {
        String name = readString(map, "name", "");
        if (name.isBlank()) return null;
        com.authmebia.notifications.Toast.Check check =
                com.authmebia.notifications.Toast.Check.parse(readString(map, "check", ""));
        if (check == null) return null;
        String contextPlayerName = PLAYER_CONTEXT.get();
        Component title = parse(replacePlayer(readString(map, "title", "Notice"), contextPlayerName));
        if (title == null) title = Component.text("Notice");
        Component content = parse(replacePlayer(readString(map, "content", ""), contextPlayerName));
        String icon = readString(map, "icon", "minecraft:paper");
        String sound = readString(map, "sound", "");
        int delay = readInt(map, "delay", 5);
        com.authmebia.notifications.Toast.Frame frame =
                com.authmebia.notifications.Toast.Frame.parse(readString(map, "frame", "task"));
    ...
    public boolean oraxenIntegrationEnabled() {
        return config.getBoolean("integrations.oraxen.enabled", false);
    }
    ...
    private String applyPlaceholderApiFromContext(String input) {
        ...
        try {
            ...
            return com.authmebia.listeners.placeholderapi.PlaceholderApi.apply(offlinePlayer, input);
        } catch (Exception e) {
            return input;
        }
    }
